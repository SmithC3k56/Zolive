...
namespace zolive_api.Services.Buyer
{
    public class BuyerService : IBuyerService
    {
        ...
        public BuyerService(newliveContext context, IUserService userService)
        {
        ...
        public async Task<List<GetGoodsOrderListModel>> getGoodsOrderList(string lan, ulong uid, string type, int p)
        {
            ...
            var shopEffectiveTime = await Utils.getShopEffectiveTime();
        ...
        public async Task goodsOrderAutoProcess(string lan, ulong uid)
        {
            ...
            var now = Utils.time();
            var effective_time = await Utils.getShopEffectiveTime();
            ...
            foreach (var v in list)
            {
                if (v.Status == 0)
                {
                    ...
                    if (pay_end <= now)
                    {
                        ...
                        await context.SaveChangesAsync();
                        await Utils.jMessageIM(title, (ulong)v.Uid, "goodsorder_admin");
                    }
                }
                if (v.Status == 1)
                {
                ...
                if (v.Status == 3 || v.Status == 4 && v.SettlementTime == 0)
                {
                    ...
                    if (settlement_end < now)
                    {
                        ...
                        await context.SaveChangesAsync();
                        #endregion
                        await Utils.jMessageIM(title, (ulong)v.ShopUid, "goodsorder_admin");
                    }
        ...
        public async Task<BalanceModel> getUserShopBalance(ulong uid)
        {
            var info = await context.CmfUsers1.Where(x => x.Id == uid).Select(x => new
            {
                balance = x.Balance,
                balance_total = x.BalanceTotal
            }).FirstOrDefaultAsync();

            BalanceModel rs = new BalanceModel();
            if (info != null)
            {
                rs.balance = info.balance;
                rs.balance_total = info.balance_total;
            }
            return rs;
        }
